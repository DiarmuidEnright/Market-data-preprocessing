...
public class MarketDataAnalyzer {
    public static void main(String[] args) {
        MarketDataAnalyzer analyzer = new MarketDataAnalyzer();
        analyzer.analyze();
    }
    ...
    public Map<String, Double> getAveragePrice(){
        return data.stream().collect(Collectors.groupingBy(MarketData::getSymbol,Collectors.averagingDouble(MarketData::getPrice)));
    }
    public Map<String, Double> getMinPrice(){
        return data.stream().collect(Collectors.groupingBy(MarketData::getSymbol,Collectors.collectingAndThen(Collectors.minBy((a,b)->Double.compare(a.getPrice(),b.getPrice())),opt->opt.get().getPrice())));
    }
    public Map<String, Double> getMaxPrice(){
        return data.stream().collect(Collectors.groupingBy(MarketData::getSymbol,Collectors.collectingAndThen(Collectors.maxBy((a,b)->Double.compare(a.getPrice(),b.getPrice())),opt->opt.get().getPrice())));
    }
    ...
    public Map<String, Long> countEntries(){
        return data.stream().collect(Collectors.groupingBy(MarketData::getSymbol,Collectors.counting()));
    }
}
